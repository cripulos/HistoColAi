...
function exporHandles(){
    console.log("Export press.............");

    console.log(ImageInfo[currentImage]["Regions"]);

    let json_export = {features: [] };

    const flatArray = ImageInfo[currentImage]["Regions"].flat();
    json_export.type="FeatureCollection";
    
    const features =[]

    flatArray.forEach((value) => {
        let coordinates = []; // Reset coordinates array for each feature
        value.path.segments.flat().forEach((val) => {
            // Adjust and round the x and y values to the desired format
            const adjustedX = Math.round(val.point.x * 100);
            const adjustedY = Math.round(val.point.y * 100);
            coordinates.push([adjustedX, adjustedY]);
        });    
        // Ensure the LinearRing is closed
        if (coordinates.length > 0 && coordinates[0].toString() !== coordinates[coordinates.length - 1].toString()) {
            coordinates.push(coordinates[0]); // Add the first point as the last point
        }
    
        // Create a GeoJSON feature with adjusted coordinates
        features.push({
            "type": "Feature",
            "geometry": {
                "type": "Polygon",
        ...
        json_export.features=features
    
        console.log(".......", JSON.stringify(json_export, null, 2));
    });
    
    

    const jsonString = JSON.stringify(json_export, null, 2);
    ...
    link.download = ImageInfo[currentImage].source.split("/").slice(-1)[0].split(".").slice(0)[0]+"_annotations_export.geojson"; // File name
...
function importHandles(){
    ...
    fileInput.addEventListener('change', async (event) => {
        ...
        try {
            ...
            const featuresArray = geoJsonData.features.map(feature => ({
                type: feature.geometry.type,
                coordinates: feature.geometry.coordinates,
                properties: feature.properties
            }));
    
            console.log("Extracted Features Array:", featuresArray);

            segments = [];
    
            // Example: Work with the array (e.g., display feature types)
            featuresArray.forEach((feature, index) => {
                // console.log(`Feature ${index + 1}:`, feature.type, feature.coordinates);
                feature.coordinates[0].forEach((points, j) =>{
                    point = { y: points[1]/100, x :points[0]/100 };
                    segments.push(point);
                }); 
            ...
            alert("GeoJSON data successfully processed!");
    
        } catch (error) {
            console.error("Error processing GeoJSON file:", error);
            alert("Failed to process the GeoJSON file. Ensure it is valid.");
        }
    }, { once: true});
    ...
    console.log(data)
		...
		if( obj ) {
			...
			for( i = 0; i < obj.Regions.length; i++ ) {
                ...
                console.log({name:reg.name,path:reg.path}, ',  type: ' , typeof({name:reg.name,path:reg.path}));
...
function inithistocolai() {
    ...
    {
        if(window.location.href.indexOf("DICOM") > -1)
            {
                appendRegionTagsFromOntology(OntologyDicom);
                appendlabaleConvictionFromOntology(OntologyDicom);
                //$("#slice-name-text").show() 
            }
        else{
            appendRegionTagsFromOntology(labels);
